...
while True:
    user_string = input('Введи строку разделенную пробелами, когда встретим любой символ отличный от цифры - выйдем')
    for i in user_string.split():
        if i.isdigit():
            summa += int(i)
        else:
            print('Найден отличный от цифры символ - выходим')
            exit()
    print(f'Сумма ранее введенных = {summa}')
